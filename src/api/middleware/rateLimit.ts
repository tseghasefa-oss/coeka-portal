...
export function rateLimiter(limit: number = 60, windowSeconds: number = 60, prefix: string = 'global') {
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    const clientIp = c.req.header('CF-Connecting-IP') || c.req.header('x-forwarded-for') || '127.0.0.1';
    const now = Math.floor(Date.now() / 1000);
    const windowSlot = Math.floor(now / windowSeconds);
    const memKey = `${prefix}:${clientIp}`;

    // If Cloudflare KV is available
    if (c.env?.RATE_LIMIT_KV) {
      try {
        const key = `ratelimit:${prefix}:${clientIp}:${windowSlot}`;
    ...
    // In-memory fallback
    const record = inMemoryRateLimitMap.get(memKey);
    if (!record || record.resetAt < now) {
      inMemoryRateLimitMap.set(memKey, { count: 1, resetAt: now + windowSeconds });
    } else {
      record.count++;
